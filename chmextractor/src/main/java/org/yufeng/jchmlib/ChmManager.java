    ...
    public byte[][] retrieveObject(FileEntry fe) {
        ...
        else if(fe.compressType==CompressType.CHM_COMPRESSED) //compressed
        {
            ...
            for(i=start;i<=bb.endBlock;) {
                ...
                i++;
                if(i%(int)clcd.resetInterval==0)lzxBlock=new LzxBlock(i,windowSize,getLzxSeg(i),lrt.block_len,null);
...
class ChmItsfHeader {
    ...
    byte[]   signature=ChmSeg.string2AsciiBytes("ITSF"); /**  0 (ITSF) 4 bytes */
    int   version;
    int   header_len;
    int   unknown_000c;
    long  last_modified;
    ...
    BigInteger   unknown_offset;  /** 38 */
    BigInteger  unknown_len;
    BigInteger  dir_offset;
    BigInteger  dir_len;
    BigInteger  data_offset;  /** 58 (Not present before V3) */
...
class ChmItspHeader {
    ...
    byte[]   signature=ChmSeg.string2AsciiBytes("ITSP"); /**  0 (ITSP) 4 bytes */
    int  version;
    int  header_len;
    int   unknown_000c;
    long  dir_blocklen;  /** 10 */
    int   blockidx_intvl;
    int  index_depth;
    int   index_root;
...
class ChmPmglSection {
    byte[]   signature=ChmSeg.string2AsciiBytes("PMGL"); /**  0 (ITSP) 4 bytes */
    long    free_space;             /**  4 */
    long    unknown_0008;
    int     block_prev;
    int     block_next;
...
class ChmPmgiSection {
    byte[]   signature=ChmSeg.string2AsciiBytes("PMGI"); /**  0 (ITSP) 4 bytes */
    long    free_space;             /**  4 */
...
class ChmLzxcControlData {
    long      size;                   /**  0        */
    byte[]  signature=ChmSeg.string2AsciiBytes("LZXC");           /**  4 (LZXC) */
    public long      version;                /**  8        */
    public long      resetInterval;          /**  c        */
    public long      windowSize;             /** 10        */
    public  long      windowsPerReset;        /** 14        */
    public long      unknown_18;             /** 18        */
